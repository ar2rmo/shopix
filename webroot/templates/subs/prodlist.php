...
<? foreach($prods as $prod) { ?>
	<li>
			...
			<div id="tovar-podsvetka-content">
				...
				<? if ($prod->price) {?><div id="tovar-price"><?=$prod->ht_full_price ?><? if ($prod->oprice) {?>&nbsp;&nbsp;<span style="text-decoration: line-through;"><?=$prod->ht_ud_price_currency ?></span><?}?></div><?}?>
				<? if (!$prod->price && (count($prod->variants_vis)>0)) { $fv=null; foreach ($prod->variants_vis as $vv) {if ($vv->price) {$fv=$vv; break;}} ?>
				<div id="tovar-price"><?=$fv->ht_full_price ?><? if ($fv->oprice) {?>&nbsp;&nbsp;<span style="text-decoration: line-through;"><?=$fv->ht_ud_price_currency ?></span><?}?></div>
				<?}?>
...
11	<ol class="tovari">
		<? foreach($prods as $prod) { ?>
			...
			<div id="tovar-podsvetka-content">
				...
				<? if ($prod->price) { ?>
				<div id="tovar-price"><?=$prod->ht_full_price ?><? if ($prod->oprice) {?> <span style="text-decoration: line-through;"><?=$prod->ht_ud_price_currency ?></span><?}?></div>
				<? } else { ?>
			...
			<div id="small-opisanie">
				<?=$prod->ht_descr_short ?>
					...
					<div id="tovar-kupit">	
						...
						<a href="" onclick="return AddToCart(document.getElementById('prodvar-<?=$prod->id?>').value,document.getElementById('cart_num_<?=$prod->id?>').value)" class="vkorzinu" title="В корзину">В&nbsp;корзину</a>
						<a href="" onclick="return AddToCmp(document.getElementById('prodvar-<?=$prod->id?>').value)" class="vkorzinu" title="К сравнению">К&nbsp;сравнению</a>
					</div>
					<? } elseif ($prod->sellable) { ?>
					<div id="tovar-kupit">	
						<input type="text" value="1" id="cart_num_<?=$prod->id?>" class="input-small3" />&nbsp;
						<a href="" onclick="return AddToCart(<?=$prod->id ?>)" class="vkorzinu" title="В корзину">В&nbsp;корзину</a>
						<a href="" onclick="return AddToCmp(<?=$prod->id ?>)" class="vkorzinu" title="К сравнению">К&nbsp;сравнению</a>
					</div>
					<? } ?>
