	...
	<? foreach ($prods as $prod) { ?>
			...
			<div id="tovar-podsvetka-content">
				...
				<? if ($prod->price) {?><div id="tovar-price"><?=$prod->ht_full_price ?><? if ($prod->oprice) {?>&nbsp;&nbsp;<span style="text-decoration: line-through;"><?=$prod->ht_oprice_currency ?></span><?}?></div><?}?>
				<? if (!$prod->price && (count($prod->variants_vis)>0)) { $fv=null; foreach ($prod->variants_vis as $vv) {if ($vv->price) {$fv=$vv; break;}} ?>
				<div id="tovar-price"><?=$fv->ht_full_price ?><? if ($prod->oprice) {?>&nbsp;&nbsp;<span style="text-decoration: line-through;"><?=$fv->ht_oprice_currency ?></span><?}?></div>
				<?}?>
