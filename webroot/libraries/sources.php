...
abstract class source_base {
	...
	public static function is_unlisted($list) {
		foreach (array_keys(static::get_all()) as $k) {
			if (!in_array($k,$list)) {
				return true;
			}
		}
...
class src_uri extends source_base implements source {
	...
	public static function trim(&$turi){
		$trimmed=false;
		$uris=self::get_all();
		$ruris=array_reverse($uris,true);
		$exclude=array();
		foreach ($ruris as $k=>$u) {
			if ($u!='') break;
			if ($k!='uri_1') $trimmed=true;
			$exclude[]=$k;
		}
	...
	public static function clear(&$turi){
		$cleared=false;
		$uris=self::get_all();
		$exclude=array();
		foreach ($uris as $k=>$u) {
			if ($u!='') continue;
			if ($k!='uri_1') $cleared=true;
			$exclude[]=$k;
		}
		foreach ($exclude as $e) {
			unset($uris[$e]);
