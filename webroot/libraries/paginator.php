...
class paginator {
	...
	public function get_parray($hide=true,$show=3) {
		$parr=array();
		...
		for ($p=1;$p<=$n;$p++) {
			if ($hide) {
				$shw=($p<($show+1)) || (abs($p-$c)<$show) || (($n-$p)<$show);
...
class paginator_overlap extends paginator {
	protected $num_overlap;
	
	function __construct($page=0,$onpage=10,$overlap=1,$items=null) {
		$this->num_overlap=$overlap;
		parent::__construct($page,$onpage,$items);
	}
	
	public function get_offset() {
		return ($this->num_onpage-$this->num_overlap)*$this->page;
	}
	
	public function get_pages() {
		if (is_null($this->num_items)) return 1;
		return ceil(($this->num_items-$this->num_overlap)/($this->num_onpage-$this->num_overlap));
	}
}
?>