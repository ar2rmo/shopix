...
class template {
	...
	private function inc($tpl) {
		...
		//extract($this->data, EXTR_PREFIX_SAME | EXTR_PREFIX_INVALID | EXTR_REFS, 'var_');
		require($this->path.$tpl.'.php');
	}
	
	private function cs($arr,$extr=null,$sep=', ') {
		if (is_null($extr)) {
			return implode($sep,$arr);
		} elseif (is_callable($extr)) {
			$sarr=array();
			foreach ($arr as $itm) {
				ob_start();
				$extr($itm);
				$sarr[]=ob_get_contents();
				ob_end_clean();
			}
			...
			foreach ($arr as $itm) {
				$fld=(string)$extr;
				if (is_object($itm)) $sarr[]=$itm->$fld;
				if (is_array($itm)) $sarr[]=$itm[$fld];
				if (is_string($itm)) $sarr[]=$itm;
			}
			return implode($sep,$sarr);
		}
	}
	
	private function h_class($arr) {
		$c=array();
		foreach ($arr as $k=>$v) {
			if ($v) $c[]=$k;
		}
	...
	public static function pgets($arr,$pref='?') {
		$gs=array();
		foreach ($arr as $key=>$val) {
			if (is_null($val)) {
				$gs[]=$key;
			} else {
				$gs[]=$key.'='.urlencode($val);
			}
		}
		if (count($gs)==0) return '';
		return $pref.implode('&',$gs);
	}
}
?>